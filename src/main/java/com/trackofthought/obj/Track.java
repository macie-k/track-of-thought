...
public class Track extends StackPane {
    ...
    private static final double fi0 = PI / 100;
    private static final String S = "straight";
    private static final String C = "curved";
    ...
    public Track(Track t) throws Exception {
        this(
            getXYFromRowCol(t.getColumn()),
            getXYFromRowCol(t.getRow()),
            t.getType(),
            t.getOrigin(),
            t.getEnd1(),
            t.getEnd2(),
    ...
    public Track(int x, int y, String type, int origin, int end1, int end2, boolean clickable)
        ...
        if (clickable) {
            ...
            /* Change the track only when mouse was released on its square */
            setOnMouseReleased(
                e -> {
        ...
        if (type.equals(S)) {
            ...
            switch (origin + currentEnd) {
                case 1:
                    ...
                    break;
                case 3:
                    quarter = (origin == 0 || currentEnd == 0) ? 4 : 2; // 2nd or 4th quarter
                    break;
                case 5:
    ...
    private int calcRotation(int origin, int end) throws Exception {
        ...
        return switch (type) {
            /* straight track */
            case S -> switch (origin + end) {
                case 2 -> 0; // (top <-> bottom)
                case 4 -> 90; // (left <-> right)
                default -> throw new Exception("Wrong 'origin'+'end' combination for straight track");
            };
            /* curved track */
            case C -> switch (origin + end) {
                case 1 -> 0; // (top -> right)
                case 3 -> (origin == 1 || origin == 2) ? 90 : 270;
                case 5 -> 180; // (bottom -> left)
                default -> throw new Exception("Wrong 'origin'+'end' combination for curved track");
            };
            default -> throw new Exception("Wrong track type");
        };
    }
