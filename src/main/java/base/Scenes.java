...
public class Scenes {
    ...
    public static final GridSquare[][] GRID = Utils.getGrid();
    private static final Map<String, String> currentProperties = new HashMap<>();
    private static final ArrayList<Map<String, String>> listMap = new ArrayList<>();
    private static final String FONT_TITLE = "Hind Guntur Bold";
    private static final String FONT_TEXT = "Poppins Light";
    ...
    public static Pane intro() {
        ...
        startBg.setOnMouseClicked(e -> moveAway.play());
    ...
    public static FullTrack gameTrack(String level, boolean premade) {
        ...
        if (Integer.parseInt(level) > 8) {
            levelCounter = 2; // temporary restriction for new levels
        }
        ...
        ArrayList<Track> clickableTracks =
            new ArrayList<>(); // smaller list for ball interactions & random switching
    ...
    */
    ...
    public static Pane createLevel() {
        ...
        allProperties = Utils.getAllJsonKeys(createObject);
        ...
        OK.setOnMouseClicked(
            e -> {
                ...
                addNewObject(root, new HashMap<>(currentProperties), menuStack);
        ...
        SAVE_NAMEClose.setOnMouseClicked(
            e -> {
                root.getChildren()
                    .forEach(
                        child -> {
                            child.setVisible(
                                !child.equals(
                                    menuStack)); // when [ x ] is clicked show
        ...
        Rectangle SAVE_NAMEButton =
            new Rectangle(
                100, 25, !levelName.getText().isEmpty() ? COLOR_ENABLED : COLOR_DISABLED);

        /* input area for entering level name */
        levelName.setId("level-name");
        levelName.setMaxSize(150, 25);
        levelName.setTranslateY(-5);
        levelName.setOnKeyReleased(
            e -> {
        ...
        CONFIRM_CREATE.setDisable(levelName.getText().isEmpty());
        CONFIRM_CREATE.setOnMouseClicked(
            e -> {
        ...
        menuObjectArrowLeft.setOnMouseClicked(
            e -> {
                objectIndex =
                    (objectIndex != 0)
                    ? objectIndex
                    : menuObjects.length; // some weird looping
                createObjectStr = menuObjects[--objectIndex]; // save current object as string
                createObject =
                    jsonObjects.getJSONObject(
                        createObjectStr); // save current object as json object
        ...
        menuObjectArrowRight.setOnMouseClicked(
            e -> {
            ...
            for (int j = 0; j < 9; j++) {
                ...
                gridSq.setOnMouseClicked(
                    e -> {
                        ...
                        //					updateMenu(menuObjectText, menuObject, menuObjectArrowLeft,
                        // menuObjectArrowRight, menuStack, menuBg, OK);
                    });
        ...
        CLEAR.setOnMouseClicked(
            e -> {
                root.getChildren().removeIf(Scenes::shouldClear);
                listMap.clear();
            });
    ...
    public static String getNextBallColor(FullTrack track) {
        ...
        final List<String> activeColorsWithoutBorders = new ArrayList<>(
            activeColors.stream().map(
                color -> color.contains("+") ? color.split("\\+")[0].trim() : color
            ).toList());

        /* get most recent ball(s) & its color */
        final List<Ball> mostRecentBalls = track.getMostRecentBalls(1);
        final String mostRecentColor =
            mostRecentBalls.isEmpty() ? "" : mostRecentBalls.get(0).getColorStr();

        for (String color : usedColors) {
            final String colorWithoutBorder =
                color.split("\\+")[0].trim(); // get current color without border
            final String prevColorWithoutBorder =
                mostRecentColor.split("\\+")[0].trim(); // get previous color without border

            final int occurence =
                Collections.frequency(
                    activeColorsWithoutBorders,
                    colorWithoutBorder); // check for occurence without border

            double probability =
                Math.pow((1.0 / usedColors.size()), occurence); // calculate probability
            ...
            if (colorWithoutBorder.equals(prevColorWithoutBorder)
                || (track.getCurrentEndStation().getColorStr().equals(color)
                && activeColors.size() > 5)) {
                probability = 0;
            }
            pairs.add(new Pair<>(color, probability));
        }
        ...
        switch (object) {
            case "track": {
                ...
                try {
                    ...
                    t.addDebugPath(
                        root); // comment to hide PATH DRAWING - can get annoying when
                    // deleting a lot

                    /* listener to remove object and change type if clickable */
                    t.setOnMouseReleased(
                        e -> {
    ...
    private static void saveLevelToJSON(String levelName) {
        try {
            ...
            obj.put(
                "tracks",
                new JSONArray()); // add tracks as object with an empty array of properties
            ...
            for (Map<String, String> m : listMap) {
                ...
                for (String key : keys) {
                    // overwrite startStation boolean if needed
                    ...
                    if (key.equals("column") || key.equals("row")) {
                        ...
                        // put key and its value from map to current JSON object
                        currObj.put(key, m.get(key));
                    }
                }
                // if its starting station force-change color to black
                if (startStation) {
                    currObj.put("color", "black");
                }
                // get JSON array of current type and add new values
    ...
    private static void updateMenu(
        ...
        menuStack
            .getChildren()
            .addAll(menuBg, ObjectText, Object, ObjArrowL, ObjArrowR, OK); // add constants
        ...
        for (String currentProperty : allProperties) {
            ...
            // current key e.g: {color, type, start, ...}
            String key = currentProperty.substring(2);
            // set current property do its default value
            ...
            /* text containing the key's default value */
            ...
            rightArrow.setOnMouseClicked(
                e -> {
                    ...
                    // update visible value text & increment index
