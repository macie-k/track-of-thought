...
public class Log {
    ...
    private static final String SUCCESS = Ansi.colorize(" [OK] ", new AnsiFormat(Attribute.GREEN_TEXT()));
    private static final String ERROR = Ansi.colorize(" [ERROR] ", new AnsiFormat(Attribute.RED_TEXT()));
    private static final String WARNING = Ansi.colorize(" [!] ", new AnsiFormat(Attribute.YELLOW_TEXT()));
    ...
    private static String getCallerLog(String log) {
        String[] callerClassSplit =
            Thread.currentThread().getStackTrace()[3].getClassName().split("\\.");
    ...
    private static String getTime() {
        return String.format(
            "[%s] \u2022", DateTimeFormatter.ofPattern("HH:mm:ss").format(LocalDateTime.now()));
    }
}
